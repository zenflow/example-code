...
describe('compile', () => {
  it('#01 basic', async () => {
    const compiledHtml = compile({
      html: `<pre><code class="executable js">let [foo, bar] = [1, 2]</code></pre>`,
    })
  ...
  it('#02 only touches <code class="executable js"> elements', async () => {
    ...
    const compiledHtml = compile({ html: originalHtml })
    expect(compiledHtml).toEqual(originalHtml)
  })

  it('#03 accepts babel configuration', async () => {
    const compiledHtml = compile({
      html: `<code class="executable js">let [foo, bar] = [1, 2]</code>`,
      babel: {
        presets: [
          [
